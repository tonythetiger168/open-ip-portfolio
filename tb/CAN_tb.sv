...
`ifdef VERILATOR
      ...
      begin int wtd; wtd = 0;
        ...
        if (wtd >= 200000) begin errors++; $display("ERROR: CAN rx_valid timeout"); end
      end
`else
...
`ifdef VERILATOR
  ...
  // =====================================================================
  localparam int CAN_FSM_TOTAL = 16;  // 8 TX + 8 RX states
...
`ifdef VERILATOR
    ...
    // 120 frames through the register interface, CAN loopback:
...
`ifdef VERILATOR
    begin
      ...
      $display("FSM_COV: %0d/%0d", visited, CAN_FSM_TOTAL);

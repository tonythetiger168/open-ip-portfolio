...
`ifdef VERILATOR
  ...
  // (minimal repro: DUT-less clk + repeat(30) @(posedge clk) -> 295 us),
...
`ifdef VERILATOR
    $display("CRV: directed done @%0t", $time);
`endif
...
`ifdef VERILATOR
    ...
    // encodings and credit-stress (requests issued immediately on each
    // acceptance-credit grant, gaps randomized down to zero).
    begin : crv_phase
      logic [11:0] c_txn;
      logic [31:0] c_addr;
      int n_req = 0;
      for (int t = 0; t < 120; t++) begin
        ...
        n_req++;
        if (n_req % 20 == 0) $display("CRV progress: %0d/120 @%0t", n_req, $time);

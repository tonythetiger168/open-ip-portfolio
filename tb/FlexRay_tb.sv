...
// Self-checking testbench: FlexRay loopback (rxd = txd) -- SystemVerilog
`timescale 1ns/1ps
module FlexRay_tb;
  logic clk = 0, rst_n = 0;
  logic rxd, txd;
...
`ifdef VERILATOR
      ...
      begin int wtd; wtd = 0;
        ...
        if (wtd >= 200000) begin errors++; $display("ERROR: FlexRay rx_valid timeout"); end
      end
`else
...
`ifdef VERILATOR
  ...
  // =====================================================================
  localparam int FlexRay_FSM_TOTAL = 16;  // 8 TX + 8 RX states
...
`ifdef VERILATOR
    ...
    // 120 frames through the register interface, FlexRay loopback:
...
`ifdef VERILATOR
    begin
      ...
      $display("FSM_COV: %0d/%0d", visited, FlexRay_FSM_TOTAL);

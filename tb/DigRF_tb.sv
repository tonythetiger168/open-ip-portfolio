...
// Self-checking testbench: TB acts as DigRF master -- SystemVerilog
`timescale 1ns/1ps
module DigRF_tb;
  logic clk = 0, rst_n = 0;
  tri1  sdata;
...
`ifdef VERILATOR
  ...
  // assertions.
  // =====================================================================
  localparam int DIGRF_FSM_TOTAL = 4;  // ST_IDLE..ST_PARK (rtl enum)
  ...
  `define DIGRF_M_BIT(b) \
    m_oe = 1; m_val = (b); #300; sclk = 1; #600; sclk = 0; #300;
  `define DIGRF_M_RBIT(b) \
    ...
    begin : crv_phase
      ...
      for (int t = 0; t < 130; t++) begin
        ...
        if (roll < 4) begin
          ...
          `DIGRF_M_SSC
          `DIGRF_M_BIT(1'b0) `DIGRF_M_BIT(1'b1) `DIGRF_M_BIT(1'b0)  // SA=01, PC=0
          for (int i = 4; i >= 0; i--) begin bb = ad[i]; `DIGRF_M_BIT(bb) end
          for (int i = 7; i >= 0; i--) begin bb = v[i];  `DIGRF_M_BIT(bb) end
          `DIGRF_M_BIT(1'b1)                                       // bus park
          m_oe = 0; #600;
          ...
          tx_byte = v;
          `DIGRF_M_SSC
          `DIGRF_M_BIT(1'b0) `DIGRF_M_BIT(1'b1) `DIGRF_M_BIT(1'b1)  // SA=01, PC=1
          for (int i = 4; i >= 0; i--) begin bb = ad[i]; `DIGRF_M_BIT(bb) end
          for (int i = 7; i >= 0; i--) begin `DIGRF_M_RBIT(bb) rd_c[i] = bb; end
          `DIGRF_M_BIT(1'b1)                                       // bus park
          m_oe = 0; #600;
          // NOTE: rd_c[0] is excluded from the compare -- documented RTL
          // bug (recorded, not fixed per v2.5 rules): DigRF_top drives
          ...
          rx_cnt_before = rx_cnt;
          `DIGRF_M_SSC
          `DIGRF_M_BIT(sa[1]) `DIGRF_M_BIT(sa[0]) `DIGRF_M_BIT(1'b0)
          for (int i = 4; i >= 0; i--) begin bb = ad[i]; `DIGRF_M_BIT(bb) end
          for (int i = 7; i >= 0; i--) begin bb = v[i];  `DIGRF_M_BIT(bb) end
          `DIGRF_M_BIT(1'b1)
          m_oe = 0; #600;
...
`ifdef VERILATOR
    begin
      int visited;
      visited = 0;
      for (int s = 0; s < DIGRF_FSM_TOTAL; s++) visited += fsm_seen[s];
      $display("FSM_COV: %0d/%0d", visited, DIGRF_FSM_TOTAL);

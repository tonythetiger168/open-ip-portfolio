...
`ifdef VERILATOR
  ...
  // assertions.
  // =====================================================================
  localparam int RFFE_FSM_TOTAL = 4;  // ST_IDLE..ST_PARK (rtl enum)
    ...
    begin : crv_phase
      ...
      for (int t = 0; t < 130; t++) begin
        ...
        if (roll < 4) begin
          ...
          `RFFE_M_SSC
          `RFFE_M_BIT(1'b0) `RFFE_M_BIT(1'b1) `RFFE_M_BIT(1'b0)  // SA=01, PC=0
          for (int i = 4; i >= 0; i--) begin bb = ad[i]; `RFFE_M_BIT(bb) end
          for (int i = 7; i >= 0; i--) begin bb = v[i];  `RFFE_M_BIT(bb) end
          `RFFE_M_BIT(1'b1)                                       // bus park
          m_oe = 0; #600;
          ...
          tx_byte = v;
          `RFFE_M_SSC
          `RFFE_M_BIT(1'b0) `RFFE_M_BIT(1'b1) `RFFE_M_BIT(1'b1)  // SA=01, PC=1
          for (int i = 4; i >= 0; i--) begin bb = ad[i]; `RFFE_M_BIT(bb) end
          for (int i = 7; i >= 0; i--) begin `RFFE_M_RBIT(bb) rd_c[i] = bb; end
          `RFFE_M_BIT(1'b1)                                       // bus park
          m_oe = 0; #600;
          // NOTE: rd_c[0] is excluded from the compare -- documented RTL
          // bug (recorded, not fixed per v2.5 rules): RFFE_top drives
          ...
          rx_cnt_before = rx_cnt;
          `RFFE_M_SSC
          `RFFE_M_BIT(sa[1]) `RFFE_M_BIT(sa[0]) `RFFE_M_BIT(1'b0)
          for (int i = 4; i >= 0; i--) begin bb = ad[i]; `RFFE_M_BIT(bb) end
          for (int i = 7; i >= 0; i--) begin bb = v[i];  `RFFE_M_BIT(bb) end
          `RFFE_M_BIT(1'b1)
          m_oe = 0; #600;
...
`ifdef VERILATOR
    begin
      int visited;
      visited = 0;
      for (int s = 0; s < RFFE_FSM_TOTAL; s++) visited += fsm_seen[s];
      $display("FSM_COV: %0d/%0d", visited, RFFE_FSM_TOTAL);

...
// Self-checking testbench: TB acts as MIPI SoundWire master -- SystemVerilog
`timescale 1ns/1ps
...
`ifdef VERILATOR
  ...
  // assertions.
  // =====================================================================
  localparam int SW_FSM_TOTAL = 4;  // ST_IDLE..ST_PARK (rtl enum)
    ...
    begin : crv_phase
      ...
      for (int t = 0; t < 130; t++) begin
        ...
        if (roll < 4) begin
          ...
          `SW_M_SSC
          `SW_M_BIT(1'b0) `SW_M_BIT(1'b1) `SW_M_BIT(1'b0)  // SA=01, PC=0
          for (int i = 4; i >= 0; i--) begin bb = ad[i]; `SW_M_BIT(bb) end
          for (int i = 7; i >= 0; i--) begin bb = v[i];  `SW_M_BIT(bb) end
          ...
          tx_byte = v;
          `SW_M_SSC
          `SW_M_BIT(1'b0) `SW_M_BIT(1'b1) `SW_M_BIT(1'b1)  // SA=01, PC=1
          for (int i = 4; i >= 0; i--) begin bb = ad[i]; `SW_M_BIT(bb) end
          ...
          rx_cnt_before = rx_cnt;
          `SW_M_SSC
          `SW_M_BIT(sa[1]) `SW_M_BIT(sa[0]) `SW_M_BIT(1'b0)
          for (int i = 4; i >= 0; i--) begin bb = ad[i]; `SW_M_BIT(bb) end
          for (int i = 7; i >= 0; i--) begin bb = v[i];  `SW_M_BIT(bb) end
          `SW_M_BIT(1'b1)
          m_oe = 0; #600;
...
`ifdef VERILATOR
    begin
      int visited;
      visited = 0;
      for (int s = 0; s < SW_FSM_TOTAL; s++) visited += fsm_seen[s];
      $display("FSM_COV: %0d/%0d", visited, SW_FSM_TOTAL);

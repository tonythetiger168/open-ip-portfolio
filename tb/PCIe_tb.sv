...
// PCIe TB: host sends flit frame, device echoes, host verifies payload.
`timescale 1ns/1ps
module PCIe_tb;
...
`ifdef VERILATOR
  ...
  // =====================================================================
  localparam int PCIe_FSM_TOTAL = 5;   // 3 TX + 2 RX states
...
`ifdef VERILATOR
    begin
      ...
      $display("FSM_COV: %0d/%0d", visited, PCIe_FSM_TOTAL);

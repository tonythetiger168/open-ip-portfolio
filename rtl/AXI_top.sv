...
module AXI_top #(
  ...
  input  logic [3:0]      awlen,    // AXI3: 4-bit length, burst <= 16
  input  logic [2:0]      awsize,
  input  logic [1:0]      awburst,
  input  logic            awvalid,
  output logic            awready,
  // ---- write data channel (AXI3: carries WID) ----
  input  logic [3:0]      wid,
  ...
  // any 4-bit ID is supported (single outstanding), so ID is not an error
  function automatic logic decerr(input logic [2:0] size,
                                  input logic [1:0] burst);
    decerr = (size > 3'd2) || (burst == 2'b11);
  endfunction
      ...
      case (wstate)
        W_IDLE: begin
          if (aw_accept) begin
            ...
            wburst_q <= awburst;
            wid_q    <= awid;
            werr_q   <= decerr(awsize, awburst)         ? RESP_DECERR :
        ...
        W_DATA: begin
          if (w_accept) begin
            ...
            // WID must match AWID: no write interleaving supported
            if (werr_q == RESP_OKAY) begin
              if (wid != wid_q) begin
                werr_q <= RESP_SLVERR;
              end else if (addr_in_range(waddr_q)) begin
      ...
      case (rstate)
        R_IDLE: begin
          if (ar_accept) begin
            ...
            rburst_q <= arburst;
            rid_q    <= arid;
            rerr_q   <= decerr(arsize, arburst)         ? RESP_DECERR :
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n)
      irq <= 1'b0;
    else if ((aw_accept && (decerr(awsize, awburst) ||
                            slverr_addr(awaddr, awburst))) ||
             (w_accept  && ((wid != wid_q) || !addr_in_range(waddr_q) ||
                            (wlast != (wbeat_q == wlen_q)))) ||
             (ar_accept && (decerr(arsize, arburst) ||
